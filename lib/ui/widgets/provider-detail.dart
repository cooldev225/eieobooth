...
class ProviderDetail extends StatefulWidget {
  ...
  final status;
  @override
  _ProviderDetail createState() => _ProviderDetail();
  ProviderDetail({@required this.guest, @required this.status});
}

class _ProviderDetail extends State<ProviderDetail>
  ...
  loadInvitations() async {
    var result = await post(
        'guest/services', {'guest': guest['_id'], 'status': status}, context);
    if (result['success']) {
      ...
      if (result) {
        ...
        if (res['success']) {
          result = await post(
              'resident/notify',
              {
                'message': preDefinedMessages(7),
                'type': 7,
                'resident_id': invitorId,
